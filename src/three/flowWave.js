...
/**
 * "Flow Wave" — a sea of glowing particle hills, recolored into the app's
 * warm graphite / amber / sage palette. Same scene as the standalone
 * flow-wave.html page, adapted to run as the main page's live background:
 * scroll is driven by the real document instead of an artificial spacer,
 * and pixel ratio is capped for a canvas that now sits behind live UI.
 */

const bgColor = '#0E0C09'; // FinalPass background tint (app --bg)
const flameColor = '#E0A75C'; // corner-flame A (app --primary-hover)
const flameColor2 = '#F2DDB0'; // corner-flame B (pale warm gold)
const flameAmt = 0.2;
const atmoColor = '#6F9C85'; // ambient motes color (app --accent)
const atmoCount = 300;
const atmoSize = 24;
const atmoSpeed = 1.0;
const colorLow = '#0E0C09'; // points low color (app --bg)
const colorHigh = '#CD9046'; // points high color (app --primary)
const opacity = 0.26;
const pointSize = 5.5;
...
const MAX_PIXEL_RATIO = 2; // background element behind live UI — cap DPR for perf
...
const finalFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    gl_FragColor = vec4(
      bg + flame * uFlameAmt + texture2D(bloomTexture, vUv).xyz + texture2D(torusTexture, vUv).xyz +
...
/**
 * Builds the whole scene against `canvas` and returns `{ resize, dispose }`.
 * `getScrollFraction` is called every frame and must return a 0..1 number —
 * the caller drives it from the real page's scroll position.
 */
export function createFlowWaveScene(canvas, { getScrollFraction }) {
  ...
  atmoGeometry.setAttribute('position', new THREE.BufferAttribute(positions, 3));
  atmoGeometry.setAttribute('size', new THREE.BufferAttribute(sizes, 1));
  atmoGeometry.setAttribute('seed', new THREE.BufferAttribute(seeds, 1));
  ...
  // Pointer input — parallax + surface repel.
  ...
  function updateScene(scroll, m) {
    ...
    pointsUniforms.uStream.value = stream;
    ...
    pointsUniforms.uAppear.value = Math.max(0, Math.min(1, (elapsed - 0.2) / 1.4));
  }
  ...
  // A background scene has no reason to keep painting (and draining battery)
  // behind a hidden tab — pause the loop until it's visible again.
  ...
  function frame() {
    ...
    const scrollTarget = clamp(getScrollFraction(), 0, 1);
